...
public abstract class ClassLoader {
    ...
    // @101142ts, 
    public static Class<?> exgetClass(String className) {
        //不需要Lz/s/MainActivity;
        //需要z.s.MainActivity

        Class<?> clazz = VMClassLoader.exfindLoadedClass(className);
        if (clazz == null) {
            try {
                clazz = VMClassLoader.loadClass(className, false);
